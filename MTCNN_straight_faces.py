...
def face_detection_MTCNN(detect_multiple_faces=False):
    ...
    for names_folder in files:
        if not names_folder.endswith('.zip'):
            new_path_in = path_in + names_folder                # new_path_in = './data/0079'
            # print(new_path_in)
            new_path_out = path_out + names_folder              # new_path_out = './output/0079'
            # print(new_path_out)
            go_to_file = os.path.join(new_path_in)              # go_to_file = './data/0079'
            # print(go_to_file)
            ...
            for names_img in name_img_file:
                ...
                # print(path_to_img)                
                path_out_img = new_path_out + "/" + names_img   #--------- ./output/1519/Image99.png
                # print("---------",path_out_img) 
                ...
                if nrof_faces > 0:                          # neu so luong khuon mat > 0
                    ...
                    for i, det in enumerate(det_arr):       # 
                        facial_points = points[i]           # muc dich vong lap nay dua ve dang 1 ngoac vuong [1,2,3,4]  
                    

                    cv2.line(img, (facial_points[2],0), (facial_points[2],facial_points[7]+200), color, 2)          #

                    x_eyes_left = facial_points[2] - facial_points[0]       #     X_mui~ - X_mat_trai
                    x_eye_right = facial_points[1] - facial_points[2]      #     X_mat_phai - X_mui
                    ...
                    if abs(sai_so_x) <= 4:
                        if abs(sai_so_y) <= 4:
                        # text = "Anh Dat "
                        # cv2.putText(img,  "{} = {:.2f}".format(text, sai_so), (int(det[0]), int(det[1]-5)), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 2, lineType=1)
                            if not os.path.exists(new_path_out):
                                os.makedirs(new_path_out)

                            copy(os.path.join(path_to_img), path_out_img)
                            print("{} --- Sai_so_x = {} --- Sai_so_y = {}".format(path_to_img, sai_so_x, sai_so_y))

                    else:
                        # text = "Chua Dat"
                        # cv2.putText(img,  "{} = {:.2f}".format(text, sai_so), (int(det[0]), int(det[1]-5)), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2, lineType=1)
                        continue
...
if __name__ == "__main__":
    path_in = "./data/"                             # input 
    path_out = "./output/"                          # output

    if not os.path.exists(path_out):                # tao fonder output
        os.makedirs(path_out)

    files = os.listdir(path_in)                     # liet ke danh sach
